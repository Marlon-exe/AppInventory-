...
const ViewIcon = (props: any) => (
    <svg aria-hidden="true" fill="none" focusable="false" height="1em" role="presentation" viewBox="0 0 20 20" width="1em" {...props}>
        ...
        <path d="M9.99999 16.8917C12.9417 16.8917 15.6833 15.1583 17.5917 12.1583C18.3417 10.9833 18.3417 9.00831 17.5917 7.83331C15.6833 4.83331 12.9417 3.09998 9.99999 3.09998C7.05833 3.09998 4.31666 4.83331 2.40833 7.83331C1.65833 9.00831 1.65833 10.9833 2.40833 12.1583C4.31666 15.1583 7.05833 16.8917 9.99999 16.8917Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} />
    </svg>
);
...
export const TableBase = ({ columns, rows, page, totalPages, onPageChange, onEdit, onDelete, onView, rowKey = 'id' }: TableProps) => {

    const columnWithActions = useMemo(() => {
        if (!onEdit && !onDelete && !onView) return columns;
        return [...columns, { key: 'acciones', label: 'Acciones' }];
    ...
    return (
        ...
        >
            <TableHeader columns={columnWithActions}>
                {(column: Column) => (
                    <TableColumn
                        key={column.key}
                        align={column.key === 'acciones' ? 'center' : 'center'}
                        className={column.key === 'acciones' ? 'w-24 text-center' : 'center'}
                    >
                        {column.label}
                    </TableColumn>
                )}
            ...
            >
                {(row) => (
                    <TableRow key={row[rowKey]}>
                        {(columnKey) => (
                            <TableCell>
                                {renderCell(row, columnKey as string)}
                            </TableCell>
                        )}
                    </TableRow>
                )}
